...
$query="update  f_accounts set firstname=?,lastname=?,dept=?,gender=?,contact=?,email=?,address=?,religion=? where regno=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sssssssss',$firstname,$lastname,$dept,$gender,$contact,$email,$address,$religion,$aregno);
$stmt->execute();
   ...
   //$cnt=1;
     while($row=$res->fetch_object())
    {
      ?>
           ...
           <div class="row">
               <div class="col-25">
               <label class="l" for="regno">Facuilty Id:</label>
           </div>
          <div class="col-75">
